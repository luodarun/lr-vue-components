...
```html
<template>
  <lr-button type="text" @click="open">点击打开 Message Box</lr-button>
</template>
...
如果你完整引入了 LrUI Vue.prototype 添加如下全局方法：$msgbox, $alert, $confirm 和 $prompt。因此在 Vue instance 中可以采用本页面中的方式调用 `MessageBox`。调用参数为：
- `$msgbox(options)`
