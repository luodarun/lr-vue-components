import { arrayFindIndex } from "lr-ui/src/utils/util";
import { getCell, getColumnByCell, getRowIdentity } from "./util";
import { getStyle, hasClass, removeClass, addClass } from "lr-ui/src/utils/dom";
...
export default {
  ...
  render(h) {
    ...
    return (
      <table class="lr-table__body" cellspacing="0" cellpadding="0" border="0">
        <colgroup>
  ...
  watch: {
    ...
    // update DOM manually. see https://github.com/LremeFE/element/pull/13954/files#diff-9b450c00d0a9dec0ffad5a3176972e40
    "store.states.hoverRow"(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll(".lr-table__row");
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      ...
      if (this.stripe && rowIndex % 2 === 1) {
        classes.push("lr-table__row--striped");
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, "lr-tooltip") && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push("lr-table__row--level-" + treeRowData.level);
    ...
    wrappedRowRender(row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        if (!renderExpanded) {
          console.error("[Lrement Error]renderExpanded is required.");
          return tr;
        }
        // 使用二维数组，避免修改 $index
        return [
          [
            ...
            <tr key={"expanded-row__" + tr.key}>
              <td colspan={this.columnsCount} class="lr-table__expanded-cell">
