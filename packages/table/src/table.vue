<template>
  <div
    ...
    :class="[
      {
        'lr-table--fit': fit,
        'lr-table--striped': stripe,
        'lr-table--border': border || isGroup,
        'lr-table--hidden': isHidden,
        'lr-table--group': isGroup,
        'lr-table--fluid-height': maxHeight,
        'lr-table--scrollable-x': layout.scrollX,
        'lr-table--scrollable-y': layout.scrollY,
        'lr-table--enable-row-hover': !store.states.isComplex,
        'lr-table--enable-row-transition':
          (store.states.data || []).length !== 0 &&
          (store.states.data || []).length < 100
      },
      tableSize ? `lr-table--${tableSize}` : ''
    ]"
  ...
  >
    ...
    <div
      ...
      class="lr-table__header-wrapper"
      ref="headerWrapper"
    >
      <table-header
        ref="tableHeader"
    ...
    >
      ...
      <div
        ...
        class="lr-table__empty-block"
        ref="emptyBlock"
        :style="{
          width: bodyWidth
        }"
      >
        <span class="lr-table__empty-text">
          <slot name="empty">{{ emptyText || "暂无数据" }}</slot>
        </span>
      </div>
      <div
        v-if="$slots.append"
        class="lr-table__append-wrapper"
        ref="appendWrapper"
      >
    ...
    <div
      ...
      v-mousewheel="handleHeaderFooterMousewheel"
      class="lr-table__footer-wrapper"
      ref="footerWrapper"
    >
      <table-footer
        :store="store"
    ...
    >
      <div
        v-if="showHeader"
        class="lr-table__fixed-header-wrapper"
      ...
      <div
        class="lr-table__fixed-body-wrapper"
      ...
      <div
        v-if="showSummary"
        v-show="data && data.length > 0"
        class="lr-table__fixed-footer-wrapper"
    ...
    <div
      v-if="rightFixedColumns.length > 0"
      class="lr-table__fixed-right-patch"
      ref="rightFixedPatch"
      :style="{
    ...
    ></div>
    <div
      class="lr-table__column-resize-proxy"
      ref="resizeProxy"
...
import Mousewheel from "lr-ui/src/directives/mousewheel";
...
export default {
  ...
  created() {
    this.tableId = "lr-table_" + tableIdSeed++;
