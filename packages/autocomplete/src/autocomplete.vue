...
import LrAutocompleteSuggestions from "./autocomplete-suggestions.vue";
import Emitter from "lr-ui/src/mixins/emitter";
import Migrating from "lr-ui/src/mixins/migrating";
import { generateId } from "lr-ui/src/utils/util";
...
export default {
  ...
  componentName: "LrAutocomplete",

  components: {
    LrInput,
    LrAutocompleteSuggestions
  },
  ...
  computed: {
    ...
    id() {
      return `lr-autocomplete-${generateId()}`;
    }
  },
  watch: {
    suggestionVisible(val) {
      ...
      if ($input) {
        this.broadcast("LrAutocompleteSuggestions", "visible", [
          val,
          $input.offsetWidth
        ]);
      }
  ...
  methods: {
    ...
    highlight(index) {
      ...
      const suggestion = this.$refs.suggestions.$el.querySelector(
        ".lr-autocomplete-suggestion__wrap"
      );
      const suggestionList = suggestion.querySelectorAll(
        ".lr-autocomplete-suggestion__list li"
      );
