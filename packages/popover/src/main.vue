<template>
  <span>
    ...
    >
      <div
        class="lr-popover lr-popper"
        :class="[popperClass, content && 'lr-popover--plain']"
        ref="popper"
      ...
      >
        <div class="lr-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
import { on, off } from "lr-ui/src/utils/dom";
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, "lr-popover__reference");
