...
import LrButtonGroup from "lr-ui/packages/button-group";
import { generateId } from "lr-ui/src/utils/util";

export default {
  name: "LrDropdown",

  componentName: "LrDropdown",
  ...
  watch: {
    visible(val) {
      this.broadcast("LrDropdownMenu", "visible", val);
      this.$emit("visible-change", val);
    },
    focusing(val) {
      const selfDefine = this.$el.querySelector(".lr-dropdown-selfdefine");
      if (selfDefine) {
        // 自定义
        if (val) {
    ...
    let triggerElm = !splitButton ? (
      ...
      <lr-button-group>
        ...
        <lr-button
          ...
          type={type}
          size={dropdownSize}
          class="lr-dropdown__caret-button"
        >
          <i class="lr-dropdown__icon lr-icon-down"></i>
        </lr-button>
      </lr-button-group>
    );

    return (
      <div class="lr-dropdown" v-clickoutside={hide}>
        {triggerElm}
        {this.$slots.dropdown}
      </div>
    );
