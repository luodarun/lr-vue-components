<template>
  <div
    class="lr-select"
    :class="[selectSize ? 'lr-select--' + selectSize : '']"
    @click.stop="toggleMenu"
    v-clickoutside="handleClose"
  >
    <div
      class="lr-select__tags"
      v-if="multiple"
      ref="tags"
    ...
    >
      <span v-if="collapseTags && selected.length">
        <lr-tag
          ...
          disable-transitions
        >
          <span class="lr-select__tags-text">{{
        ...
        <lr-tag
          ...
          disable-transitions
        >
          <span class="lr-select__tags-text">+ {{ selected.length - 1 }}</span>
        </lr-tag>
      </span>
      <transition-group @after-leave="resetInputHeight" v-if="!collapseTags">
        <lr-tag
          ...
          disable-transitions
        >
          <span class="lr-select__tags-text">{{ item.currentLabel }}</span>
        </lr-tag>
      </transition-group>

      <input
        type="text"
        class="lr-select__input"
    ...
    >
      ...
      <template slot="suffix">
        ...
        <i
          v-if="showClose"
          class="lr-select__caret lr-input__icon lr-icon-closecircleo"
    ...
    >
      ...
      >
        <lr-scrollbar
          tag="ul"
          wrap-class="lr-select-dropdown__wrap"
          view-class="lr-select-dropdown__list"
          ref="scrollbar"
          :class="{
        ...
        >
          <lr-option :value="query" created v-if="showNewOption"> </lr-option>
          <slot></slot>
        </lr-scrollbar>
        <template
          v-if="
        ...
        >
          ...
          <p class="lr-select-dropdown__empty" v-else>
            {{ emptyText }}
          </p>
        </template>
      </lr-select-menu>
...
import Focus from "lr-ui/src/mixins/focus";
import LrInput from "lr-ui/packages/input";
import LrSelectMenu from "./select-dropdown.vue";
import LrOption from "./option.vue";
import LrTag from "lr-ui/packages/tag";
...
import scrollIntoView from "lr-ui/src/utils/scroll-into-view";
import {
  getValueByPath,
  valueEquals,
  isIE,
  isEdge
} from "lr-ui/src/utils/util";
import NavigationMixin from "./navigation-mixin";
import { isKorean } from "lr-ui/src/utils/shared";
...
export default {
  ...
  watch: {
    ...
    visible(val) {
      if (!val) {
        this.broadcast("LrSelectDropdown", "destroyPopper");
        if (this.$refs.input) {
        ...
        this.broadcast("LrSelectDropdown", "updatePopper");
        if (this.filterable) {
          ...
          if (this.multiple) {
            ...
            if (!this.remote) {
              this.broadcast("LrOption", "queryChange", "");
              this.broadcast("LrOptionGroup", "queryChange");
            }
  ...
  methods: {
    ...
    handleQueryChange(val) {
      ...
      this.previousQuery = val;
      this.$nextTick(() => {
        if (this.visible) this.broadcast("LrSelectDropdown", "updatePopper");
      });
      this.hoverIndex = -1;
      ...
      if (this.remote && typeof this.remoteMethod === "function") {
        ...
        this.broadcast("LrOption", "queryChange", val);
        this.broadcast("LrOptionGroup", "queryChange");
      }
      if (
        this.defaultFirstOption &&
